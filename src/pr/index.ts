...
export function generatePRBody(options: PRBodyOptions): string {
  ...
  // Footer
  sections.push('\n---');
  sections.push('*Generated with [git-ship](https://github.com/thisismayank/git-ship)*');

  return sections.join('\n');
}

export async function getRecentCommits(baseBranch: string, headBranch: string): Promise<string[]> {
  try {
...
export interface AIGeneratePRBodyOptions {
  issue: IssueContext | null;
  commits: CommitGroup[];
  branchName: string;
  baseBranch: string;
...
function buildPRPrompt(options: AIGeneratePRBodyOptions): string {
...
async function callOpenAIForPR(prompt: string, model: string): Promise<string> {
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a senior software engineer who writes clear, professional pull request descriptions.',
      },
...
async function callAnthropicForPR(prompt: string, model: string): Promise<string> {
  ...
  const response = await client.messages.create({
    ...
    system: 'You are a senior software engineer who writes clear, professional pull request descriptions.',
  });
...
export async function generatePRBodyWithAI(options: AIGeneratePRBodyOptions): Promise<string> {
