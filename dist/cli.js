...
import { createLinearClient } from './linear/index.js';
import { heuristicGroup, mergeAIGroups } from './analysis/grouper.js';
import { analyzeWithAI } from './analysis/ai-client.js';
import { formatAllCommitMessages } from './analysis/commit-message.js';
import { runReview, hasCriticalFindings } from './review/runner.js';
...
const TOTAL_STEPS = 7;
function getReviewAdapter(tool) {
    switch (tool) {
...
async function ship(options) {
    ...
    let issueId = options.issue ?? null;
    if (!issueId) {
    ...
    if (issueId) {
        ...
        if (linearClient) {
            try {
                issue = await withSpinner('Fetching issue details', () => linearClient.getIssue(issueId));
                if (issue) {
                ...
                logger.warn(`Could not fetch Linear issue: ${error.message}`);
    ...
    const diffs = await withSpinner('Parsing diffs', () => getFileDiffs(git, filePaths));
    ...
    let groups;
    try {
        const aiGroups = await withSpinner('AI analyzing diffs for optimal grouping', () => analyzeWithAI(config, diffs, preGroups, issue));
        groups = mergeAIGroups(preGroups, aiGroups, filePaths);
    }
    catch (error) {
        logger.warn(`AI grouping failed: ${error.message}`);
    ...
    if (action === 'regroup') {
        ...
        try {
            const aiGroups = await withSpinner('Re-analyzing diffs', () => analyzeWithAI(config, diffs, preGroups, issue));
    ...
    if (!skipReview) {
        ...
        try {
            const reviewResult = await withSpinner(`Running ${adapter.name} review`, () => runReview(adapter, 'main'));
            ...
            logger.warn(`Review failed: ${error.message}`);
    ...
    const pushResult = await withSpinner(`Pushing ${branchName} to origin`, () => pushToRemote(git, branchName, { setUpstream: !status.tracking }));
